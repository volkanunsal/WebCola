...
export class GridRouter<Node> {
  ...
  private getGridLines(axis): GridLine[] {
    ...
    while (ls.length > 0) {
      ...
      let overlapping = ls.filter((v) =>
        v.rect["overlap" + axis.toUpperCase()](ls[0].rect)
      );
      let col = {
        nodes: overlapping,
  ...
  constructor(
    ...
    this.nodes = originalnodes.map(
      (v, i) =>
        new NodeWrapper(i, accessor.getBounds(v), accessor.getChildren(v))
    );
    ...
    var hlines = this.rows
      .map((r) => <any>{ x1: rowx, x2: rowX, y1: r.pos, y2: r.pos })
      .concat(rowMids.map((m) => <any>{ x1: rowx, x2: rowX, y1: m, y2: m }));

    // vertical lines
    var vlines = this.cols
      .map((c) => <any>{ x1: c.pos, x2: c.pos, y1: coly, y2: colY })
  ...
  siblingObstacles(a, b) {
    ...
    var obstacles = path.commonAncestor.children.filter(
  ...
  route(s: number, t: number): Point[] {
    var source = this.nodes[<number>s],
      target = this.nodes[<number>t];
    ...
    var shortestPathCalculator = new Calculator(
      this.verts.length,
      this.passableEdges,
      getSource,
      getTarget,
