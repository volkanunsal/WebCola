...
export interface LayoutInputNode extends InputNode {
...
export interface LayoutGroup {
  bounds?: Rectangle;
  leaves?: LayoutInputNode[];
  groups?: LayoutGroup[];
  padding?: number;
}

function isLayoutGroup(g: any): g is LayoutGroup {
  return typeof g.leaves !== "undefined" || typeof g.groups !== "undefined";
}

export interface LayoutLink<NodeRefType> {
  source: NodeRefType;
  target: NodeRefType;
...
export type LinkNumericPropertyAccessor = (
  t: LayoutLink<LayoutInputNode | number>
) => number;

export interface LinkLengthTypeAccessor
  extends LinkLengthAccessor<LayoutLink<LayoutInputNode | number>> {
...
export class Layout {
  ...
  private _links: LayoutLink<LayoutInputNode | number>[] = [];
  ...
  groups(x: Array<LayoutGroup>): this;
  groups(x?: Array<LayoutGroup>): any {
    ...
    this._groups.forEach((g) => {
      ...
      if (typeof g.leaves !== "undefined") {
        g.leaves.forEach((v, i) => {
          if (typeof v === "number") (g.leaves[i] = this._nodes[v]).parent = g;
        });
      }
    ...
    this._rootGroup.leaves = this._nodes.filter(
      (v) => typeof v.parent === "undefined"
    );
    this._rootGroup.groups = this._groups.filter(
      (g) => typeof g.parent === "undefined"
    );
    return this;
  }

  powerGraphGroups(f: Function): this {
    var g = powergraph.getGroups(
      this._nodes,
      this._links,
      this.linkAccessor,
      this._rootGroup
   ...
   * @default empty list
   */
  links(): Array<LayoutLink<LayoutInputNode | number>>;
  links(x: Array<LayoutLink<LayoutInputNode | number>>): this;
  links(x?: Array<LayoutLink<LayoutInputNode | number>>): any {
  ...
  getLinkLength(link: LayoutLink<LayoutInputNode | number>): number {
  ...
  static setLinkLength(
    link: LayoutLink<LayoutInputNode | number>,
    length: number
  ) {
    link.length = length;
  }

  getLinkType(link: LayoutLink<LayoutInputNode | number>): number {
  ...
  linkAccessor: LinkLengthTypeAccessor = {
    ...
    getType: (l) =>
      typeof this._linkType === "function" ? this._linkType(l) : 0,
  };
  ...
  symmetricDiffLinkLengths(idealLength: number, w: number = 1): this {
    ...
    this._linkLengthCalculator = () =>
      symmetricDiffLinkLengths(this._links, this.linkAccessor, w);
    return this;
  }
  ...
  jaccardLinkLengths(idealLength: number, w: number = 1): this {
    this.linkDistance((l) => idealLength * l.length);
    this._linkLengthCalculator = () =>
      jaccardLinkLengths(this._links, this.linkAccessor, w);
    return this;
  }
  ...
  start(
    ...
    if (this._distanceMatrix) {
      ...
      this._links.forEach((l) => {
        if (typeof l.source == "number")
          l.source = this._nodes[<number>l.source];
        if (typeof l.target == "number")
          l.target = this._nodes[<number>l.target];
      });
    ...
    if (this._directedLinkConstraints) {
      (<any>(
        this.linkAccessor
      )).getMinSeparation = this._directedLinkConstraints.getMinSeparation;
      curConstraints = curConstraints.concat(
        generateDirectedEdgeConstraints(
          n,
          this._links,
          this._directedLinkConstraints.axis,
          <any>this.linkAccessor
        )
      );
    ...
    if (curConstraints.length > 0)
      this._descent.project = new Projection(
        this._nodes,
        this._groups,
        this._rootGroup,
        curConstraints
  ...
  private initialLayout(iterations: number, x: number[], y: number[]) {
    if (this._groups.length > 0 && iterations > 0) {
      ...
      var edges = this._links.map(
        (e) =>
          <any>{
            source: (<LayoutInputNode>e.source).index,
            target: (<LayoutInputNode>e.target).index,
          }
      );
      ...
      this._groups.forEach((g, i) => {
        if (typeof g.leaves !== "undefined")
          g.leaves.forEach((v) =>
            edges.push({ source: g.index, target: v.index })
          );
        if (typeof g.groups !== "undefined")
          g.groups.forEach((gg) =>
            edges.push({ source: g.index, target: gg.index })
          );
      });
  ...
  routeEdge(edge, ah: number = 5, draw) {
    ...
    if (shortestPath.length === 1 || shortestPath.length === vg2.V.length) {
      let route = makeEdgeBetween(
        edge.source.innerBounds,
        edge.target.innerBounds,
        ah
      );
  ...
  static getSourceIndex(e: LayoutLink<LayoutInputNode | number>): number {
    return typeof e.source === "number"
      ? <number>e.source
      : (<LayoutInputNode>e.source).index;
  }

  //The link source and target may be just a node index, or they may be references to nodes themselves.
  static getTargetIndex(e: LayoutLink<LayoutInputNode | number>): number {
    return typeof e.target === "number"
      ? <number>e.target
      : (<LayoutInputNode>e.target).index;
  }

  // Get a string ID for a given link.
  static linkId(e: LayoutLink<LayoutInputNode | number>): string {
  ...
  static dragStart(d: LayoutInputNode | LayoutGroup) {
    if (isLayoutGroup(d)) {
  ...
  private static stopNode(v: LayoutInputNode) {
    (<any>v).px = v.x;
    (<any>v).py = v.y;
  }
  ...
  private static storeOffset(d: LayoutGroup, origin: { x: number; y: number }) {
