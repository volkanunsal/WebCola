...
class ShortestPathNode {
  ...
  neighbours: ShortestPathNeighbour[];
  d: number;
  prev: ShortestPathNode;
  q: PairingHeap<ShortestPathNode>;
}

class QueueEntry {
  constructor(
    public node: ShortestPathNode,
    public prev: QueueEntry,
    public d: number
  ) {}
}
...
export class Calculator<Link> {
  private neighbours: ShortestPathNode[];
  ...
  constructor(
    ...
    getLength: (l: Link) => number
  ) {
    this.neighbours = new Array(this.n);
    var i = this.n;
    while (i--) this.neighbours[i] = new ShortestPathNode(i);

    i = this.es.length;
    while (i--) {
      var e = this.es[i];
      var u: number = getSourceIndex(e),
        v: number = getTargetIndex(e);
      var d = getLength(e);
      this.neighbours[u].neighbours.push(new ShortestPathNeighbour(v, d));
      this.neighbours[v].neighbours.push(new ShortestPathNeighbour(u, d));
    }
  }
  ...
  PathFromNodeToNodeWithPrevCost(
    ...
    prevCost: (u: number, v: number, w: number) => number
  ): number[] {
    var q = new PriorityQueue<QueueEntry>((a, b) => a.d <= b.d),
      u: ShortestPathNode = this.neighbours[start],
  ...
  private dijkstraNeighbours(start: number, dest: number = -1): number[] {
    var q = new PriorityQueue<ShortestPathNode>((a, b) => a.d <= b.d),
      i = this.neighbours.length,
      d: number[] = new Array(i);
    while (i--) {
      var node: ShortestPathNode = this.neighbours[i];
