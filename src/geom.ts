...
export function clockwiseRadialSweep(
  p: Point,
  P: Point[],
  f: (p: Point) => void
) {
  P.slice(0)
    .sort(
      (a, b) =>
        Math.atan2(a.y - p.y, a.x - p.x) - Math.atan2(b.y - p.y, b.x - p.x)
    )
    .forEach(f);
}
...
function tangent_PointPolyC(
  P: Point,
  V: Point[]
): { rtan: number; ltan: number } {
...
export function tangent_PolyPolyC(
  V: Point[],
  W: Point[],
  t1: (a: Point, b: Point[]) => number,
  t2: (a: Point, b: Point[]) => number,
  cmp1: (a: Point, b: Point, c: Point) => boolean,
  cmp2: (a: Point, b: Point, c: Point) => boolean
): { t1: number; t2: number } {
...
export function LRtangent_PolyPolyC(
  V: Point[],
  W: Point[]
): { t1: number; t2: number } {
...
export function RLtangent_PolyPolyC(
  V: Point[],
  W: Point[]
): { t1: number; t2: number } {
  return tangent_PolyPolyC(
    V,
    W,
    Rtangent_PointPolyC,
    Ltangent_PointPolyC,
...
export function LLtangent_PolyPolyC(
  V: Point[],
  W: Point[]
): { t1: number; t2: number } {
  return tangent_PolyPolyC(
...
export function RRtangent_PolyPolyC(
  V: Point[],
  W: Point[]
): { t1: number; t2: number } {
  return tangent_PolyPolyC(
...
export class TangentVisibilityGraph {
  ...
  constructor(
    public P: TVGPoint[][],
    g0?: { V: VisibilityVertex[]; E: VisibilityEdge[] }
  ) {
    if (!g0) {
      var n = P.length;
      // For each node...
