...
export class Descent {
  ...
  public static createSquareMatrix(
    n: number,
  ...
  public computeDerivatives(x: number[][]) {
    ...
    for (let u = 0; u < n; ++u) {
      ...
      for (let v = 0; v < n; ++v) {
        ...
        if (
          (weight > 1 && distance > idealDistance) ||
          !isFinite(idealDistance)
        ) {
        ...
        const idealDistSquared = idealDistance * idealDistance,
          gs =
            (2 * weight * (distance - idealDistance)) /
            (idealDistSquared * distance),
        ...
        for (i = 0; i < this.k; ++i) {
          ...
          Huu[i] -= this.H[i][u][v] =
            hs *
            (2 * distanceCubed + idealDistance * (d2[i] - distanceSquared));
        }
      }
      for (i = 0; i < this.k; ++i)
  ...
  private static mid(a: number[][], b: number[][], m: number[][]): void {
    Descent.mApply(
      a.length,
      a[0].length,
      (i, j) => (m[i][j] = a[i][j] + (b[i][j] - a[i][j]) / 2.0)
    );
  }
