export class PairingHeap<T> {
  ...
  public count(): number {
    return this.empty()
      ? 0
      : 1 +
          this.subheaps.reduce((n: number, h: PairingHeap<T>) => {
            return n + h.count();
          }, 0);
  }
  ...
  public isHeap(lessThan: (a: T, b: T) => boolean): boolean {
    return this.subheaps.every(
      (h) => lessThan(this.elem, h.elem) && h.isHeap(lessThan)
    );
  }
...
export class PriorityQueue<T> {
  ...
  public push(...args: T[]): PairingHeap<T> {
    ...
    for (var i = 0, arg; (arg = args[i]); ++i) {
      ...
      this.root = this.empty()
        ? pairingNode
        : this.root.merge(pairingNode, this.lessThan);
    }
    return pairingNode;
  }
  /**
  ...
  public reduceKey(
    ...
    newKey: T,
    setHeapNode: (e: T, h: PairingHeap<T>) => void = null
  ): void {
    this.root = this.root.decreaseKey(
      heapNode,
      newKey,
      setHeapNode,
      this.lessThan
