...
export class Layout3D {
  ...
  constructor(
    public nodes: Node3D[],
    public links: Link3D[],
    public idealLinkLength: number = 1
  ) {
  ...
  start(iterations: number = 100): Layout3D {
    ...
    const distanceMatrix = new Calculator(
      n,
      this.links,
      (e) => e.source,
      (e) => e.target,
    ...
    this.links.forEach(
      ({ source, target }) => (G[source][target] = G[target][source] = 1)
    );
    ...
    //});
    if (this.constraints)
      this.descent.project = new Projection(
        <GraphNode[]>this.nodes,
        null,
        null,
        this.constraints
      ).projectFunctions();
...
class LinkAccessor implements LinkLengthAccessor<any> {
  getSourceIndex(e: any): number {
    return e.source;
  }
  getTargetIndex(e: any): number {
    return e.target;
  }
  getLength(e: any): number {
    return e.length;
  }
  setLength(e: any, l: number) {
    e.length = l;
  }
}
