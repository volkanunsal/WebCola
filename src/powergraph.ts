...
export class PowerEdge {
  constructor(public source: any, public target: any, public type: number) {}
}
...
export class Configuration<Link> {
  ...
  constructor(
    n: number,
    edges: Link[],
    private linkAccessor: LinkTypeAccessor<Link>,
    rootGroup?: any[]
  ) {
      ...
      for (var j = 0; j < group.groups.length; ++j) {
        ...
        moduleSet.add(
          new Module(
            ...
            new LinkSets(),
            new LinkSets(),
            this.initModulesFromGroup(child),
            definition
          )
        );
      }
  ...
  greedyMerge(): boolean {
    for (var i = 0; i < this.roots.length; ++i) {
      ...
      var ms = this.rootMerges(i).sort((a, b) =>
        a.nEdges == b.nEdges ? a.id - b.id : a.nEdges - b.nEdges
      );
      var m = ms[0];
    ...
    if (m.isLeaf()) {
      ...
      if (!m.isIsland() || m.isPredefined()) {
        ...
        if (m.isPredefined())
          ...
          for (var prop in m.definition) g[prop] = m.definition[prop];
...
export class LinkSets {
  ...
  add(linktype: number, m: Module) {
    var s: ModuleSet =
      linktype in this.sets
        ? this.sets[linktype]
        : (this.sets[linktype] = new ModuleSet());
    s.add(m);
    ++this.n;
  }
...
export function getGroups<Link>(
  nodes: any[],
  links: Link[],
  la: LinkTypeAccessor<Link>,
  rootGroup?: any[]
): { groups: any[]; powerEdges: PowerEdge[] } {
  var n = nodes.length,
