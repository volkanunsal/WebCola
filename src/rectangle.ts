...
export function computeGroupBounds(g: ProjectionGroup): Rectangle {
  g.bounds =
    typeof g.leaves !== "undefined"
      ? g.leaves.reduce(
          (r: Rectangle, c) => c.bounds.union(r),
          Rectangle.empty()
        )
      : Rectangle.empty();
  if (typeof g.groups !== "undefined")
    g.bounds = <Rectangle>(
      g.groups.reduce(
        (r: Rectangle, c) => computeGroupBounds(c).union(r),
        g.bounds
      )
    );
...
export class Rectangle {
  ...
  lineIntersections(
    ...
    var intersections = [];
    for (var i = 0; i < 4; ++i) {
      var r = Rectangle.lineIntersection(
        x1,
        y1,
        x2,
        y2,
 ...
 */
export function makeEdgeBetween(
  source: Rectangle,
  target: Rectangle,
  ah: number
...
var xRect: RectAccessors = {
  ...
  makeRect: (open, close, center, size) =>
    new Rectangle(center - size / 2, center + size / 2, open, close),
...
var yRect: RectAccessors = {
  ...
  getSize: (r) => r.height(),
  makeRect: (open, close, center, size) =>
    new Rectangle(open, close, center - size / 2, center + size / 2),
  findNeighbours: findYNeighbours,
};

function generateGroupConstraints(
  ...
  var padding = root.padding,
    ...
    childConstraints: Constraint[] = !gn
      ...
      : root.groups.reduce(
          (ccs: Constraint[], g) =>
            ccs.concat(generateGroupConstraints(g, f, minSep, true)),
          []
        ),
  ...
  if (gn)
    root.groups.forEach((g) => {
      ...
      add(
        f.makeRect(f.getOpen(b), f.getClose(b), f.getCentre(b), f.getSize(b)),
        g.minVar
      );
    });
...
export class Projection {
  ...
  constructor(
    ...
    if (avoidOverlaps && rootGroup && typeof rootGroup.groups !== "undefined") {
      ...
      var i = nodes.length;
      groups.forEach((g) => {
        this.variables[i] = g.minVar = new IndexedVariable(
          i++,
          typeof g.stiffness !== "undefined" ? g.stiffness : 0.01
        );
        this.variables[i] = g.maxVar = new IndexedVariable(
          i++,
          typeof g.stiffness !== "undefined" ? g.stiffness : 0.01
        );
      });
  ...
  private makeFeasible(c: any) {
    ...
    var vs: GraphNode[] = c.offsets
      .map((o) => this.nodes[o.node])
  ...
  xProject(x0: number[], y0: number[], x: number[]) {
    ...
    this.project(
      ...
      (v) =>
        v.bounds.setXCentre(
          (x[(<IndexedVariable>v.variable).index] = v.variable.position())
        ),
      (g) => {
  ...
  yProject(x0: number[], y0: number[], y: number[]) {
    ...
    this.project(
      ...
      (v) =>
        v.bounds.setYCentre(
          (y[(<IndexedVariable>v.variable).index] = v.variable.position())
        ),
      (g) => {
