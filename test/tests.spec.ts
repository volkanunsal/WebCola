...
const duplicate_nodes = {
  ...
  links: [{ source: 0, target: 2, value: 1 }],
};
...
describe("dom tests", () => {
  ...
  test("small power-graph", () => {
    ...
    var es;
    expect(
      c.R == (es = c.allEdges()).length ||
        "c.R=" + c.R + ", actual edges in c=" + es.length
    ).toBe(true);
  ...
  test("Graph with duplicate node name", function () {
    ...
    d3cola.nodes(nodes).links(duplicate_nodes.links).start(30);
