...
describe("Geom", () => {
  test("convex hulls", () => {
    ...
    var nextInt = function (r) {
      return Math.round(rand.getNext() * r);
    };
    var width = 100,
      height = 100;
    ...
    for (var k = 0; k < 10; ++k) {
      ...
      expect(
        h[0] !== h[h.length - 1] ||
          "first and last point of hull are different" + k
      ).toBe(true);
    }
  });
  ...
  function makePoly(rand) {
    ...
    loop: for (var i = 0; i < n; ++i) {
      ...
      var ctr = 0;
      while (
        (i > 0 && length(P[i - 1], p) < 1) || // min segment length is 1
  ...
  test("tangent visibility graph", () => {
    ...
    for (var tt = 0; tt < 1; tt++) {
      ...
      var shortestPath = new Calculator(
        g.V.length,
        g.E,
        getSource,
        getTarget,
      ...
      if (draw) {
        ...
        for (var i = 0; i < shortestPath.length; i++) {
          ...
          drawLine(
            svg,
            { x1: u.p.x, y1: u.p.y, x2: v.p.x, y2: v.p.y },
            "orange"
          );
        }
    ...
    for (var j = 0; j < 100; j++) {
      ...
      if (draw) {
        ...
        for (var p in t) {
          ...
          expect(
            ints.length <= 4 || ints.length + " intersects found at " + j
          ).toBe(true);
        }
      }
    }
