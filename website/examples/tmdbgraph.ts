...
module tmdb {
  class NodeType {
    ...
    makeEdge(thisName: string, otherName: string): Edge {
      return this === Movie
        ? new Edge(thisName, otherName)
        : new Edge(otherName, thisName);
    }
  }

  export var Movie = new NodeType("movie", "credits", "title", "posters");
  export var Person = new NodeType(
    "person",
    "movie_credits",
    "name",
    "profiles"
  ...
  export class Graph {
    ...
    expandNeighbours(node: Node, f: (v: Node) => void): JQueryPromise<Node[]> {
      var dn = node.cast.map((c) =>
        this.getNode(node.type.next(), c.id, (v) => {
    ...
    fullyExpanded(node: Node): boolean {
      return (
        node.cast &&
        node.cast.every((v) => node.type.next() + v.id in this.nodes)
      );
    }
...
module tmdbgraph2 {
  ...
  var d3cola = cola.d3adaptor(d3).linkDistance(60).size([width, height]);
  ...
  function redraw(transition) {
    ...
    (transition ? vis.transition() : vis).attr(
      "transform",
      d3.zoomTransform(vis.node()).toString()
    );
  }
  ...
  function refocus(focus) {
    var neighboursExpanded = modelgraph.expandNeighbours(
      focus,
      function (v: ViewNode) {
  ...
  function addViewNode(v: ViewNode, startpos?) {
    ...
    $.when(d).then(function (node: ViewNode) {
      d3.select("#" + node.name())
        ...
        .attr("xlink:href", function (v: any) {
          var url = v.imgurl;
          var simg = this;
          var img = new Image();
          img.onload = function () {
            simg.setAttribute("width", (nodeWidth - 4).toString());
            simg.setAttribute("height", (nodeHeight - 4).toString());
          };
          return (img.src = url);
        })
        .on("click", function () {
  ...
  function update() {
    ...
    var node = nodesLayer
      .selectAll(".node")
      .data(viewgraph.nodes, function (d: any) {
  ...
  function graphBounds() {
    ...
    nodesLayer.selectAll(".node").each(function (v: any) {
  ...
  function zoomToFit() {
    ...
    var w = b.X - b.x,
      h = b.Y - b.y;
    var cw = Number(outer.attr("width")),
      ch = Number(outer.attr("height"));
    var s = Math.min(cw / w, ch / h);
    var tx = -b.x * s + ((cw / s - w) * s) / 2,
      ty = -b.y * s + ((ch / s - h) * s) / 2;
    zoom.translateBy(vis, tx, ty);
    zoom.scaleBy(vis, s);
    redraw(true);
  }
  ...
  $().ready(() => {
    ...
    $("#fullScreenButton").click(() => {
      const elem = outer.node() as SVGSVGElement;
