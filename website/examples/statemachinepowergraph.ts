  ...
  function flatGraph() {
    ...
    d3.json(graphfile, function (error, graph: { nodes; links }) {
      ...
      var swatches = svg
        ...
        .append("rect")
        .attr("x", indent)
        .attr("y", (l, i) => topindent + 40 * i)
        .attr("width", 30)
        .attr("height", 30)
        .attr("fill", (l, i) => color(i))
        .attr("class", "swatch");
  ...
  function powerGraph() {
    ...
    d3.json(graphfile, function (error, graph: { nodes; links }) {
      ...
      var doLayout = function (response) {
        ...
        var n = graph.nodes.length,
          ...
          g = {
            ...
            edges: powerGraph.powerEdges.map(
              (e) =>
                <any>{
                  source: _id(e.source),
                  target: _id(e.target),
        ...
        var node = svg
          ...
          .attr(
            ...
            (d) =>
              (d.y +=
                d.name == "starting_state" || d.name == "trip_cancelled"
                  ? 30
                  : 0)
          )
      ...
      d3cola
        ...
        .powerGraphGroups((d) =>
          (powerGraph = d).groups.forEach((v) => (v.padding = 10))
        );
      ...
      if (
        document.URL.toLowerCase().indexOf("marvl.infotech.monash.edu") >= 0
      ) {
        ...
        d3.json(
          graphfile.replace(/.json/, "pgresponse.json"),
