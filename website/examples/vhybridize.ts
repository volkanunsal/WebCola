...
*/
  ...
  function flatGraph() {
    ...
    var swatches = svg
      ...
      .attr("class", "swatch");
    var swatchlabels = svg
      .selectAll(".swatchlabel")
      .data(linkTypes.list)
      .enter()
      .append("text")
      .text(function (t) {
    ...
    modules = {
      N: inputjson.nodes.length,
      nodes: [],
      edges: [],
    };
    inputjson.links.forEach((e) => {
      modules.edges.push({
        source: getId(e.source, inputjson.nodes.length),
        target: getId(e.target, inputjson.nodes.length),
      });
    });
  ...
  function powerGraph2(callback) {
    ...
    for (var i = 0; i < es.length; i++) {
      ...
      for (var j = i + 1; j < es.length; j++) {
        var f = es[j];
        dupFound =
          (getId(e.source, n) == getId(f.source, n) &&
            getId(e.target, n) == getId(f.target, n)) ||
          (getId(e.target, n) == getId(f.source, n) &&
            getId(e.source, n) == getId(f.target, n));
        if (dupFound) break;
      }
    ...
    pgLayout.cola
      .on("tick", function () {
        ...
        group.each(function (d) {
          d.innerBounds = d.bounds.inflate(-groupMargin);
        });
      ...
      .on("end", function () {
        var n = pgLayout.cola.nodes().length,
          ...
          g = {
            nodes: pgLayout.cola
              ...
              .concat(
                pgLayout.powerGraph.groups.map(
                  (d) =>
                    <any>{
                      id: _id(d),
                      children: (typeof d.groups !== "undefined"
                        ? d.groups.map((c) => n + c.id)
                        : []
                      ).concat(
                        typeof d.leaves !== "undefined"
                          ? d.leaves.map((c) => c.index)
                          : []
                      ),
                    }
                )
              ),
            edges: pgLayout.powerGraph.powerEdges.map(
              (e) =>
                <any>{
                  source: _id(e.source),
                  target: _id(e.target),
        ...
        var gridrouter = new cola.GridRouter(
          g.nodes,
          {
        ...
        var routes = gridrouter.routeEdges<any>(
          g.edges,
          5,
          (e) => e.source,
          (e) => e.target
          ...
          if (route.length > 1) {
            for (var i = 0; i < route.length; i++) {
              ...
              if (i < route.length - 1) {
                ...
                dy = y1 - y0;
                var angle =
                  cola.GridRouter.angleBetween2Lines(li, l) < 0 ? 1 : 0;
    ...
    noparentnodes.forEach((np) => {
      noparentnodes.forEach((npv) => {
        if (npv.index != np.index) {
          if (
            npv.bounds.X <= np.bounds.x &&
            ((npv.bounds.y <= np.bounds.y && npv.bounds.Y >= np.bounds.y) ||
              (np.bounds.y <= npv.bounds.y && np.bounds.Y >= npv.bounds.y))
          ) {
            left.push({ source: getId(npv, N), target: getId(np, N) });
          }
          if (
            npv.bounds.Y <= np.bounds.y &&
            ((npv.bounds.x <= np.bounds.x && npv.bounds.X >= np.bounds.x) ||
              (np.bounds.x <= npv.bounds.x && np.bounds.X >= npv.bounds.x))
          ) {
    ...
    noparentgroups.forEach((np) => {
      noparentgroups.forEach((npp) => {
        if (getId(npp, N) != getId(np, N)) {
          if (
            npp.bounds.X - 20 <= np.bounds.x + 20 &&
            ((npp.bounds.y + 20 <= np.bounds.y &&
              npp.bounds.Y - 20 >= np.bounds.y) ||
              (np.bounds.y <= npp.bounds.y + 20 &&
                np.bounds.Y >= npp.bounds.y + 20))
          ) {
            left.push({ source: getId(npp, N), target: getId(np, N) });
          }
          if (
            npp.bounds.Y - 20 <= np.bounds.y + 20 &&
            ((npp.bounds.x + 20 <= np.bounds.x &&
              npp.bounds.X - 20 >= np.bounds.x) ||
              (np.bounds.x <= npp.bounds.x + 20 &&
                np.bounds.X >= npp.bounds.x + 20))
          ) {
      ...
      noparentnodes.forEach((npv) => {
        if (
          npv.bounds.X <= np.bounds.x + 20 &&
          ((npv.bounds.y <= np.bounds.y + 20 &&
            npv.bounds.Y >= np.bounds.y + 20) ||
            (np.bounds.y + 20 <= npv.bounds.y &&
              np.bounds.Y - 20 >= npv.bounds.y))
        ) {
          left.push({ source: getId(npv, N), target: getId(np, N) });
        } else if (
          np.bounds.X - 20 <= npv.bounds.x &&
          ((npv.bounds.y <= np.bounds.y + 20 &&
            npv.bounds.Y >= np.bounds.y + 20) ||
            (np.bounds.y + 20 <= npv.bounds.y &&
              np.bounds.Y - 20 >= npv.bounds.y))
    ...
    parentnodes.forEach((np) => {
      parentnodes.forEach((npv) => {
        if (
          npv.index != np.index &&
          getId(npv.parent, N) == getId(np.parent, N)
        ) {
          if (
            npv.bounds.X <= np.bounds.x &&
            ((npv.bounds.y <= np.bounds.y && npv.bounds.Y >= np.bounds.y) ||
    ...
    parentgroups.forEach((np) => {
      parentgroups.forEach((npp) => {
        if (
          getId(npp, N) != getId(np, N) &&
          getId(npp.parent, N) == getId(np.parent, N)
        ) {
          if (
            npp.bounds.X - 20 <= np.bounds.x + 20 &&
            ((npp.bounds.y + 20 <= np.bounds.y + 20 &&
              npp.bounds.Y - 20 >= np.bounds.y + 20) ||
              (np.bounds.y + 20 <= npp.bounds.y + 20 &&
                np.bounds.Y - 20 >= npp.bounds.y + 20))
          ) {
            left.push({ source: getId(npp, N), target: getId(np, N) });
          }
          if (
            ...
            ((npp.bounds.x + 20 <= np.bounds.x + 20 &&
              npp.bounds.X - 20 >= np.bounds.x + 20) ||
              (np.bounds.x + 20 <= npp.bounds.x + 20 &&
                np.bounds.X - 20 >= npp.bounds.x + 20))
          ) {
            above.push({ source: getId(npp, N), target: getId(np, N) });
          }
  ...
  function alladjacency(callback) {
    ...
    pgLayout.cola.on("tick", function () {
      ...
      group.each(function (d) {
        d.innerBounds = d.bounds.inflate(-20);
      });
      link.each(function (d) {
        d.route = cola.makeEdgeBetween(
          d.source.innerBounds,
      ...
      link
        ...
        .attr("x2", function (d) {
          return d.route.targetIntersection.x;
        })
        .attr("y2", function (d) {
          return d.route.targetIntersection.y;
        });
    ...
    noparentnodes.forEach((np) => {
      myobjects.push({
        id: getId(np, N),
        x: np.bounds.x + (np.bounds.X - np.bounds.x) / 2,
        y: np.bounds.y + (np.bounds.Y - np.bounds.y) / 2,
      });
    });
    ...
    for (var i = 0; i < myobjects.length - 1; i++) {
      left.push({
        source: myobjects[i].id,
        target: myobjects[i + 1].id,
      });
    }
    myobjects.sort(function (a, b) {
    ...
    parentgroups.forEach((np) => {
      ...
      parentgroups.forEach((npp) => {
        if (
          ...
          myobjects.push({
            id: getId(npp, N),
            x: npp.bounds.x + (npp.bounds.X - npp.bounds.x) / 2,
            y: npp.bounds.y + (npp.bounds.Y - npp.bounds.y) / 2,
          });
        }
      });
      parentnodes.forEach((npv) => {
        if (getId(npv.parent, N) == getId(np.parent, N)) {
          myobjects.push({
            id: getId(npv, N),
            x: npv.bounds.x + (npv.bounds.X - npv.bounds.x) / 2,
            y: npv.bounds.y + (npv.bounds.Y - npv.bounds.y) / 2,
          });
        }
      });
