...
var undoStack = [];
var animate = false;
var touchStart = window.navigator.msPointerEnabled
  ? "MSPointerDown"
  : "touchstart";
var touchMove = window.navigator.msPointerEnabled
  ? "MSPointerMove"
  : "touchmove";
...
function undo() {
  if (undoStack.length > 0) {
    ...
    prev.forEach(function (r) {
      rs.push(
        draw.rect(r.w, r.h).move(r.x, r.y).attr("fill-opacity", 0.5).draggable()
      );
    });
  }
}
