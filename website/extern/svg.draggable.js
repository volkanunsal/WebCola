...
SVG.extend(SVG.Element, {
  ...
  draggable: function (constraint) {
    ...
    start = function (event) {
      ...
      if (element.beforedrag) element.beforedrag(event);
      ...
      if (element.dragstart)
        element.dragstart(
          { x: 0, y: 0, zoom: element.startPosition.zoom },
          event
        );
    ...
    drag = function (event) {
      ...
      if (element.startEvent) {
        ...
        if (constraint.minX != null && x < constraint.minX) x = constraint.minX;
        else if (constraint.maxX != null && x > constraint.maxX - width)
          x = constraint.maxX - width;

        if (constraint.minY != null && y < constraint.minY) y = constraint.minY;
        ...
        /* invoke any callbacks */
        if (element.dragmove) element.dragmove(delta, event);
      }
    };
    ...
    /* when dragging ends */
      ...
      /* invoke any callbacks */
      if (element.dragend) element.dragend(delta, event);
    };
