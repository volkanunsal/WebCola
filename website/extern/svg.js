...
(function () {
  this.SVG = function (element) {
    if (SVG.supported) return new SVG.Doc(element);
  };
  ...
  SVG.Array = function (array, fallback) {
    ...
    if (this.value.length == 0 && fallback) this.value = fallback;
  };
  ...
  SVG.extend(SVG.Number, {
    ...
    toString: function () {
      return (
        (this.unit == "%" ? ~~(this.value * 1e8) / 1e6 : this.value) + this.unit
      );
  ...
  SVG.extend(SVG.Element, {
    ...
    attr: function (a, v, n) {
      if (a == null) {
        ...
        for (n = v.length - 1; n >= 0; n--)
          a[v[n].nodeName] = SVG.regex.test(v[n].nodeValue, "isNumber")
        ...
        if (a == "x" && Array.isArray(this.lines))
          for (n = this.lines.length - 1; n >= 0; n--) this.lines[n].attr(a, v);
        ...
        if (SVG.Color.test(v) || SVG.Color.isRgb(v)) v = new SVG.Color(v);
        /* ensure correct numeric values */ else if (typeof v === "number")
          v = new SVG.Number(v);
        /* parse array values */ else if (Array.isArray(v))
          v = new SVG.Array(v);
        ...
        if (this._isStyle(a)) {
          ...
          /* rebuild if required */
          if (this.rebuild) this.rebuild(a, v);
        }
      }
    ...
    transform: function (o, v) {
      if (arguments.length == 0) {
        ...
        if (arguments.length < 2) return this.trans[o];
      ...
      /* merge values */
      for (v in o) if (o[v] != null) this.trans[v] = o[v];
      ...
      /* add skew on x axis */
      if (o.skewX != 0) transform.push("skewX(" + o.skewX + ")");

      /* add skew on y axis */
      if (o.skewY != 0) transform.push("skewY(" + o.skewY + ")");

      /* add translation */
      if (o.x != 0 || o.y != 0)
        transform.push(
          "translate(" +
            new SVG.Number(o.x / o.scaleX) +
            " " +
            new SVG.Number(o.y / o.scaleY) +
            ")"
        );
      ...
      if (transform.length == 0) this.node.removeAttribute("transform");
      else this.node.setAttribute("transform", transform.join(" "));
    ...
    style: function (s, v) {
      ...
      s = "";
      for (v in this.styles) s += v + ":" + this.styles[v] + ";";

      /* apply style */
      if (s == "") this.node.removeAttribute("style");
      else this.node.setAttribute("style", s);
  ...
  SVG.extend(SVG.Container, {
    ...
    clear: function () {
      ...
      /* remove defs node */
      if (this._defs) this._defs.clear();
  ...
  SVG.extend(SVG.FX, {
    ...
    animate: function (d, ease, delay) {
      ...
      fx.to = function (pos) {
        ...
        if (tkeys == null) {
          tkeys = [];
          for (key in fx.trans) tkeys.push(key);
        }
        ...
        if (fx._x) element.x(fx._at(fx._x, pos));
        else if (fx._cx) element.cx(fx._at(fx._cx, pos));

        /* run all y-position properties */
        if (fx._y) element.y(fx._at(fx._y, pos));
        else if (fx._cy) element.cy(fx._at(fx._cy, pos));

        /* run all size properties */
        if (fx._size)
          element.size(
            fx._at(fx._size.width, pos),
            fx._at(fx._size.height, pos)
          );
    ...
    attr: function (a, v, n) {
      if (typeof a == "object") for (var key in a) this.attr(key, a[key]);
      else this.attrs[a] = { from: this.target.attr(a), to: v };
    ...
    transform: function (o, v) {
      if (arguments.length == 1) {
        ...
        /* store matrix values */
        for (v in o) this.trans[v] = { from: this.target.trans[v], to: o[v] };
      } else {
        /* apply transformations as object if key value arguments are given*/
        var transform = {};
        transform[o] = v;
    ...
    style: function (s, v) {
      if (typeof s == "object") for (var key in s) this.style(key, s[key]);
      else this.styles[s] = { from: this.target.style(s), to: v };
  ...
  SVG.on = function (node, event, listener) {
    ...
    else node.attachEvent("on" + event, listener);
  };
  ...
  SVG.off = function (node, event, listener) {
    ...
    else node.detachEvent("on" + event, listener);
  };
  ...
  SVG.extend(SVG.Element, {
    ...
    maskWith: function (element) {
      ...
      this.masker =
        element instanceof SVG.Mask ? element : this.parent.mask().add(element);
  ...
  SVG.extend(SVG.Element, {
    ...
    clipWith: function (element) {
      ...
      this.clipper =
        element instanceof SVG.Clip ? element : this.parent.clip().add(element);
  ...
  SVG.extend(SVG.Gradient, {
    ...
    radius: function (r) {
      return this.type == "radial" ? this.attr({ r: new SVG.Number(r) }) : this;
    },
    // Add a color stop
    at: function (stop) {
  ...
  SVG.Doc = function (element) {
    ...
    this.constructor.call(
      this,
      this.parent.nodeName == "svg" ? this.parent : SVG.create("svg")
    );
    ...
    if (this.parent.nodeName != "svg") this.stage();
  };
  ...
  SVG.extend(SVG.Ellipse, {
    ...
    x: function (x) {
      return x == null
        ...
        : this.cx(x + this.attr("rx"));
    },
    // Move over y-axis
    y: function (y) {
      return y == null
        ? this.cy() - this.attr("ry")
        : this.cy(y + this.attr("ry"));
    },
    // Move by center over x-axis
    cx: function (x) {
      return x == null
        ? this.attr("cx")
        : this.attr("cx", new SVG.Number(x).divide(this.trans.scaleX));
    },
    // Move by center over y-axis
    cy: function (y) {
      return y == null
        ? this.attr("cy")
        : this.attr("cy", new SVG.Number(y).divide(this.trans.scaleY));
    },
  ...
  SVG.extend(SVG.Line, {
    ...
    size: function (width, height) {
      ...
      return this.attr(
        this.attr("x1") < this.attr("x2") ? "x2" : "x1",
        b.x + width
  ...
  SVG.extend(SVG.Polyline, SVG.Polygon, {
    // Private: Native plot
    _plot: function (p) {
      if (Array.isArray(p)) p = new SVG.Array(p, [[0, 0]]);
  ...
  SVG.extend(SVG.Container, {
    ...
    image: function (source, width, height) {
      ...
      return this.put(
        new SVG.Image()
          .load(source)
          .size(width, height != null ? height : width)
      );
    },
  });
  ...
  SVG.extend(SVG.Text, {
    ...
    cy: function (y, a) {
      return y == null
        ? this.bbox().cy
        : this.y(a ? y : y - this.bbox().height / 2);
    },
    // Move element to given x and y values
    move: function (x, y, a) {
    ...
    text: function (text) {
      /* act as getter */
      if (text == null) return this.content;
    ...
    tspan: function (text) {
      ...
      /* add style if any */
      if (!SVG.regex.isBlank.test(style)) tspan.style(style);
    ...
    leading: function (value) {
      ...
      /* apply leading */
      this.lines.each(function () {
        if (this.newLined) this.attr("dy", value);
      });
  ...
  ["fill", "stroke"].forEach(function (method) {
    ...
    extension[method] = function (o) {
      ...
      if (
        ...
        this.attr(method, o);
      /* set all attributes from _fillAttr and _strokeAttr list */ else
        for (index = SVG["_" + method].length - 1; index >= 0; index--)
          if (o[SVG["_" + method][index]] != null)
            this.attr(
              _colorPrefix(method, SVG["_" + method][index]),
              o[SVG["_" + method][index]]
            );
  ...
  SVG.Set.inherit = function () {
    ...
    for (var m in SVG.FX.prototype)
      if (
        ...
        typeof SVG.SetFX.prototype[m] != "function"
      )
        methods.push(m);
    ...
    methods.forEach(function (method) {
      SVG.SetFX.prototype[method] = function () {
        for (var i = 0, il = this.set.members.length; i < il; i++)
          this.set.members[i].fx[method].apply(
            this.set.members[i].fx,
            arguments
          );
  ...
  SVG.extend(SVG.Element, {
    ...
    remember: function (k, v) {
      ...
      /* retrieve memory */ else if (arguments.length == 1)
        return this._memory[k];
      /* store memory */ else this._memory[k] = v;
  ...
  else if (typeof exports !== "undefined") exports.SVG = SVG;
}.call(this));
